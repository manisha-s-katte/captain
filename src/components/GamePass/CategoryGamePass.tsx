...
const CategoryCard: React.FC<CategoryCardProps> = ({ title, imageSrc, text1, text2 }) => {
  return (
    <>
    ...
    <div className="border-2 border-[#FF41B3] max-w-sm">
      ...
      <div className=" mb-2">
        <Image src={imageSrc} alt={title} width={100} height={100} className="w-full h-64 object-cover " />
      </div>
      <div className="flex flex-row p-2 px-4 justify-between">
        <p className="text-sm mb-2">{text1}</p>
        <button className="text-sm px-4 py-0.5 outline-none border-none font-semibold rounded-sm border-2 bg-[#D600E1] text-white transition-colors">{text2}</button>
      </div>
    </div>
    </div>
...
const CategoryGamePass: React.FC = () => {
  ...
  return (
    <div className="flex flex-col  min-h-screen  p-4 gap-8">
